...
namespace Project
{
    public partial class User : Form
    {
        ...
        public User(string user_id, int user_code)
        {
            InitializeComponent();
            id = user_id;
            userCode = user_code;
        ...
        private void btn_book_rental_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (booklist.Items[selectRow].SubItems[4].Text == "0")
                {
                    MessageBox.Show("이 도서는 현재 수량이 없습니다.");
                    return;
                }
                Rentalform subForm = new Rentalform(isbn, bookName, userCode, id);
                subForm.ShowDialog();
            }
        ...
        private void btn_book_return_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string bookName = booklist.Items[selectRow].SubItems[0].Text;
                string isbn = booklist.Items[selectRow].SubItems[2].Text;
                ReturnForm subForm = new ReturnForm(isbn, bookName, userCode, id);
                subForm.ShowDialog();
